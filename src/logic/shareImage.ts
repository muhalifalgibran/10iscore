...
export async function generateShareImage(
  ...
  const headerH = 56;
  const rowH = Math.min(72, (H - panelY - 160 - headerH) / Math.max(maxRows, 1));
  ...
  ctx.fillText('Scored on 10iScore', W / 2, H - 70);
  ctx.fillStyle = 'rgba(216,242,74,0.6)';
  ctx.font = '500 14px "Geist", -apple-system, sans-serif';
  drawSpacedText(ctx, 'GAME · SET · MATCH', W / 2, H - 40, 4);
